...
namespace CGC.Advent.Core.Helpers
{
    ...
    public class AmplifierHelper
    {
        ...
        private static int GetMaxFeedbackThrust(List<List<int>> phaseSettings, string intcodeSource)
        {
            //< Process each setting and get the result, retain all results
            var resultMap = new Dictionary<List<int>, int>();
            foreach (var phaseSetting in phaseSettings)
            {
                ...
                //< Second, go through and assign the connectivity of the Amplifiers
                for (int i = 0; i < NumAmplifiers; i++)
                {
                    //< Assign the connectivity
                    int connectedIdx = i < phaseSetting.Count - 1 ? i + 1 : 0;
                    amps[i].OutputAmp = amps[connectedIdx];
                }

                //< We need to just give the first Amplifier an input of '0'
                amps.First().AddInput(0);
                //< Then run, and it'll cascade through.. somehow
                amps.First().Process();

                //< Then, we want to collect the last output of the last amp
                var output = amps.Last().Output;
                resultMap.Add(phaseSetting, (int)output);
            }
        ...
        private static int GetMaxFeedback(List<List<int>> phaseSettings, string intcodeSource)
        {
            //< Process each setting and get the result, retain all results
            var resultMap = new Dictionary<List<int>, int>();
            foreach (var phaseSetting in phaseSettings)
